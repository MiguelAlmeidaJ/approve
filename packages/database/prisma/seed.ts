...
async function createItem(input: {
  ...
  return prisma.contentItem.create({
    data: {
      ...
      subheadline: "Conteúdo demonstrativo para visualizar o fluxo completo.",
      designerNotes: "Dados fictícios gerados pelo seed de demonstração.",
...
async function main() {
  const dev = await upsertUser({
    name: process.env.DEV_NAME?.trim() || "Dev Terceiro Andar",
    email: process.env.DEV_EMAIL?.trim() || "designer@dev.com",
    password: process.env.DEV_PASSWORD || "senha123",
    role: UserRole.DEV,
  ...
  for (const [name, contentType, width, height, supportsFeed, supportsStories] of formatSeeds) {
